  ...
  return (
    <div className='currentWeather'>
        <div className='currentWeather-inner'>
            ...
            <div className='day'>{dateFormat(data.current?.dt)}, {timeFormat(data.current?.dt)}{" "}</div>
            <div className='overcast'>{data.current?.weather[0]?.description}</div>
            <div className='clouds'> {data.current?.weather[0]?.main} {`${data.current?.clouds}%`}</div>
            <div className='img-banner'>
              <img  src={require("../../img/banner.webp")}>
              </img>
